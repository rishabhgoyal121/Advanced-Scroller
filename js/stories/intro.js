function intro_slide() {
  d3.selectAll(
    "#intro-matching-background-animation, #intro-matching-foreground-animation, #intro-matching-data, #intro-matching-style, #intro-landing-static"
  )
    .transition()
    .duration(0)
    .attr("display", "block")
  ...
  d3.select("#intro-matching-foreground-animation")
    .selectAll(".bar")
    .data(bars)
    .enter()
    .append("rect")
  ...
  d3.select("#intro-matching-foreground-animation")
    ...
    .attr("height", 50)
    .attr("x", function (d, i) {
      return 418 - 35 + 56.5 * (i - 3 * d);
    })
    .attr("y", function (d, i) {
      return 299.5 - 50 - 58 * d;
    })
    .attr("xlink:href", function (d, i) {
      return "img/stylist" + (i + 1) + ".svg";
    })
    .style("opacity", 1)
  ...
  for (var i = 0; i < 100; i++) {
    ...
    d3.select("#intro-matching-background-animation")
      ...
      .attr(
        "transform",
        "translate(" + coord_x_client(rad) + "," + (svg_px_height + 50) + ")"
      )
      .remove();
  }
  for (var i = 0; i < 150; i++) {
  ...
  function client_style_match_animation() {
    ...
    if (Math.random() < 0.25) {
      ...
      if (style_outline_n == 7) {
        style_outline_n = 10;
      }
      if (style_outline_n == 8) {
        style_outline_n = 13;
      }
    }
    ...
    d3.select("#intro-client" + client_outline_n)
      .attr(
        "transform",
        "translate(-40," + -1 * intro_foreground_top_padding + ")"
      )
      .style("opacity", 1)
      .transition()
      .duration(duration / 3)
    ...
    d3.select("#intro-matching-style")
      .append("path")
      .attr("fill", "#fff")
      .attr("stroke", "#000")
    ...
    var client_data_timer = d3.timer(function (t) {
      if (t > duration) {
        ...
        d3.select(".intro-matching-data-client").text(
          client_data.substring(
            0,
            Math.floor(
              (client_data.length * (t - duration / 3)) / (duration / 6)
            )
          )
        );
    ...
    var style_data_timer = d3.timer(function (t) {
      if (t > duration) {
        ...
        d3.select(".intro-matching-data-style").text(
          style_data.substring(
            0,
            Math.floor(
              (style_data.length * (t - duration / 3)) / (duration / 6)
            )
          )
        );
      } else if (t > duration / 2 && t < (duration * 2) / 3) {
        d3.select(".intro-matching-data-style")
          ...
          .text(
            style_data.substring(
              0,
              Math.floor(
                (style_data.length * (-t + (duration * 2) / 3)) / (duration / 6)
              )
            )
          );
      } else {
    ...
    d3.selectAll(
      "#intro-vector-axes, #intro-c-vector, #intro-s-vector, #intro-dotted-vector, #intro-c-text, #intro-s-text"
    )
      .transition()
      .delay((duration * 6.5) / 12)
      .duration(duration / 6)
      .style("opacity", 1)
      .transition()
      .delay((duration * 5) / 6)
  ...
  animation_state.intro_matching_background_interval = setInterval(
    intro_background_animation,
    700
  );
  intro_background_animation();

  animation_state.intro_matching_interval = setInterval(
...
function stop_intro_animation() {
  ...
  d3.select("#intro-matching-background-animation")
    .selectAll(".client-cycle-silhouette")
    .transition()
    .duration(0)
    .remove();

  d3.select("#intro-matching-background-animation")
    .selectAll(".inventory-cycle-silhouette")
    .transition()
    .duration(0)
    .remove();
